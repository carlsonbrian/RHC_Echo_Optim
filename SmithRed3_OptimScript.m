...
%   ventricular-ventricular interaction, valve inertances and all dead space/zero 
%   pressure volumes (except in pericardium) have been removed.
%
%   Model originally created on     17   January 2016
%   Model last modfied on           19 Septebmer 2018
...
% ***********************************************************************************
    ...
    if (SmithParam_Flag == 1)
        ...
        if (OptimBest_Flag == 0)
            ...
            P_0_pcd = 0.5003;                       % Unstress pericard press (mmHg)
            lambda_pcd = 0.03;                      % Pericard press exp term (1/mL)
            V_0_pcd = 200 * 2.00;                   % Pericard zero P volume (mL)
            P_th = 0; %-4;                          % Thoracic pressure (mmHg)
            % Left ventricle free wall parameters
            E_es_lvf = 2.8798 * 0.585;              % LV free wall elast (mmHg/mL) 
            P_0_lvf = 0.1203;                       % LV ED pressure param (mmHg)
            lambda_lvf = 0.033 * 0.525;              % LV ED pressure param (1/mL)
            % Right ventricle free wall parameters
            E_es_rvf = 0.585 * 0.75;                % RV free wall elast (mmHg/mL) 
            P_0_rvf = 0.2157;                       % RV ED pressure param (mmHg)
            lambda_rvf = 0.023 * 0.55;              % RV ED pressure param (1/mL)
            % Pulmonary artery and vein parameters
            E_es_pa = 0.369 * 0.70;                 % Pulm artery elastance (mmHg/mL)
            E_es_pu = 0.0073 * 75.00;               % Pulm vein elastance (mmHg/mL)
            R_pul = 0.1552 * 1.25;                  % Pulm vasc resist (mmHg*s/mL)
            % Aortic and vena cava parameters
            ...
            % Save all parameters in a sturcture to pass
            CVParam_Values = {period A B C P_0_pcd lambda_pcd V_0_pcd ...
                P_th E_es_lvf P_0_lvf lambda_lvf E_es_rvf P_0_rvf ...
                lambda_rvf E_es_pa E_es_pu R_pul E_es_ao E_es_vc ...
                R_sys R_mt R_av R_tc R_pv SVFact};
            CVParam_Fields = {'period' 'A' 'B' 'C' 'P_0_pcd' 'lambda_pcd' ...
                'V_0_pcd' 'P_th' 'E_es_lvf' 'P_0_lvf' 'lambda_lvf' ...
                'E_es_rvf' 'P_0_rvf' 'lambda_rvf' 'E_es_pa' 'E_es_pu' ...
                'R_pul' 'E_es_ao' 'E_es_vc' 'R_sys' 'R_mt' 'R_av' ...
                'R_tc' 'R_pv' 'SVFact'};
            CVParam_Struct = cell2struct(CVParam_Values,CVParam_Fields,2);
...
% ***********************************************************************************
    ...
    % 6 --> P_0_lvf         LV ED pressure param (mmHg)
    % 7 --> lambda_lvf      LV ED pressure param (1/mL)
    % 8 --> E_es_rvf        RV free wall elast (mmHg/mL)        RIGHT VENTRICLE 
    % 9 --> P_0_rvf         RV ED pressure param (mmHg)
    % 10 -> lambda_rvf      RV ED pressure param (1/mL)
    % 11 -> E_es_pa         Pulm artery elastance (mmHg/mL)     PULMONARY 
    % 12 -> E_es_pu         Pulm vein elastance (mmHg/mL)       VASCULATURE
    % 13 -> R_pul           Pulm vasc resist (mmHg*s/mL)
    % 14 -> E_es_ao         Aorta elastance (mmHg/mL)           SYSTEMIC 
    % 15 -> E_es_vc         Vena cava elastance (mmHg/mL)       VASCULATURE
    % 16 -> R_sys           Syst art resistance (mmHg*s/mL)
    % 17 -> R_mt            Mitral valve resist (mmHg*s/mL)     HEART VALVES
    % 18 -> R_av            Aortic valve resist (mmHg*s/mL)
    % 19 -> R_tc            Tricspd vlv resist (mmHg*s/mL)
    % 20 -> R_pv            Pulmon vlv resist (mmHg*s/mL)
    % 21 -> SVFact          Stressed blood vol factor (uls)     CIRC BLOOD VOLUME
    ...
    LowBp_All(6) = log(0.01);                       % LV ED pressure param, P_0_lvf
    UpBp_All(6) = log(5.00);
    LowBp_All(7) = log(0.005);                      % LV ED press param, lambda_lvf
    UpBp_All(7) = log(0.100);
    LowBp_All(8) = log(0.050);     % RIGHT         % RV free wall elast, E_es_rvf
    UpBp_All(8) = log(5.000);      %  VENTRICLE
    LowBp_All(9) = log(0.01);                      % RV ED pressure param, P_0_rvf
    UpBp_All(9) = log(5.00);
    LowBp_All(10) = log(0.005);                     % RV ED press param, lambda_rvf
    UpBp_All(10) = log(0.100);
    LowBp_All(11) = log(0.050);     % PULMONARY     % Pulm artery elastance, E_es_pa
    UpBp_All(11) = log(5.000);      %  VASCULATURE
    LowBp_All(12) = log(0.0005);                    % Pulm vein elastance, E_es_pu
    UpBp_All(12) = log(0.1000);
    LowBp_All(13) = log(0.005);                     % Pulm vasc resist, R_pul
    UpBp_All(13) = log(1.000);
    LowBp_All(14) = log(0.05);      % SYSTEMIC      % Aorta elastance, E_es_ao
    UpBp_All(14) = log(5.00);       %  VASCULATURE
    LowBp_All(15) = log(0.0001);                    % Vena cava elastance, E_es_vc
    UpBp_All(15) = log(0.1000);
    LowBp_All(16) = log(0.05);                      % Syst art resistance, R_sys
    UpBp_All(16) = log(15);
    LowBp_All(17) = log(0.005);     % HEART         % Mitral valve resist, R_mt
    UpBp_All(17) = log(0.500);      %  VALVES
    LowBp_All(18) = log(0.005);                     % Aortic valve resist, R_av
    UpBp_All(18) = log(0.500);
    LowBp_All(19) = log(0.005);                     % Tricspd vlv resist, R_tc
    UpBp_All(19) = log(0.500);
    LowBp_All(20) = log(0.0005);                    % Pulmon vlv resist, R_pv
    UpBp_All(20) = log(0.2500);
    LowBp_All(21) = log(0.1);       % CIRC          % Stress blood vol fact, SVFact
    UpBp_All(21) = log(3);          %  BLOOD VOL                          
...
% ***********************************************************************************
    ...
    if (SmithParam_Flag == 1)
        ...
        % Solve over the time span with ode15s
        [T_OutSS,X_OutSS] = ode15s(@dXdT_SmithRed3, ...
            TSpan_SS,X0,[],CVParam_Struct);
        % Now solve over the simulation time span
        [T_Out,X_Out] = ode15s(@dXdT_SmithRed3, ...
...
% ***********************************************************************************
    ...
    if (SmithParam_Flag == 1)
        ...
        for i = 1:Num_TOut
            VarOut = dXdT_SmithRed3(T_Out(i),X_Out(i,:),CVParam_Struct,1);
            P_LVSim(i) = VarOut(1);
            P_RVSim(i) = VarOut(2);
            P_AOSim(i) = VarOut(3);
            P_VCSim(i) = VarOut(4);
