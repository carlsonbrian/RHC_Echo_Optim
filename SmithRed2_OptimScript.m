...
%   ventricular-ventricular interaction and valve inertances have been removed.
%
    ...
    if (SmithParam_Flag == 1)
        ...
        if (OptimBest_Flag == 0)
            ...
            CVParam_Values = {period A B C P_0_pcd lambda_pcd V_0_pcd ...
                P_th E_es_lvf V_d_lvf P_0_lvf lambda_lvf V_0_lvf E_es_rvf ...
                V_d_rvf P_0_rvf lambda_rvf V_0_rvf E_es_pa V_d_pa E_es_pu ...
                V_d_pu R_pul E_es_ao V_d_ao E_es_vc V_d_vc R_sys R_mt ...
                R_av R_tc R_pv SVFact};
            CVParam_Fields = {'period' 'A' 'B' 'C' 'P_0_pcd' 'lambda_pcd' ...
                'V_0_pcd' 'P_th' 'E_es_lvf' 'V_d_lvf' 'P_0_lvf' ...
                'lambda_lvf' 'V_0_lvf' 'E_es_rvf' 'V_d_rvf' 'P_0_rvf' ...
                'lambda_rvf' 'V_0_rvf' 'E_es_pa' 'V_d_pa' 'E_es_pu' ...
                'V_d_pu' 'R_pul' 'E_es_ao' 'V_d_ao' 'E_es_vc' 'V_d_vc' ...
...
% ***********************************************************************************
    ...
    % 26 -> R_av            Aortic valve resist (mmHg*s/mL)
    % 27 -> R_tc            Tricspd vlv resist (mmHg*s/mL)
    % 28 -> R_pv            Pulmon vlv resist (mmHg*s/mL)
    % 29 -> SVFact          Stressed blood vol factor (uls)     CIRC BLOOD VOLUME
    ...
    LowBp_All(26) = log(0.005);                     % Aortic valve resist, R_av
    UpBp_All(26) = log(0.500);
    LowBp_All(27) = log(0.005);                     % Tricspd vlv resist, R_tc
    UpBp_All(27) = log(0.500);
    LowBp_All(28) = log(0.0005);                    % Pulmon vlv resist, R_pv
    UpBp_All(28) = log(0.2500);
    LowBp_All(29) = log(0.1);       % CIRC          % Stress blood vol fact, SVFact
    UpBp_All(29) = log(3);          %  BLOOD VOL                          
...
% ***********************************************************************************
    ...
    if (SmithParam_Flag == 1)
        ...
        % Solve over the time span with ode15s
        [T_OutSS,X_OutSS] = ode15s(@dXdT_SmithRed2, ...
            TSpan_SS,X0,[],CVParam_Struct);
        % Now solve over the simulation time span
        [T_Out,X_Out] = ode15s(@dXdT_SmithRed2, ...
...
% ***********************************************************************************
    ...
    if (SmithParam_Flag == 1)
        ...
        for i = 1:Num_TOut
            VarOut = dXdT_SmithRed2(T_Out(i),X_Out(i,:),CVParam_Struct,1);
            P_LVSim(i) = VarOut(1);
            P_RVSim(i) = VarOut(2);
            P_AOSim(i) = VarOut(3);
            P_VCSim(i) = VarOut(4);
